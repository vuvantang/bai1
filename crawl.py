...
class Crawl:
    ...
    def get_laptop_from_url(url):
        ...
        return [LaptopModel(Process.get_product_name(div.select_one('h2.product-row-name').get_text()),
                            Process.get_brand_name(div.select_one('h2.product-row-name').get_text()),
                            Process.convert_text_to_number(div.select_one('del').get_text()),
                            Process.convert_text_to_number(div.select_one('span.product-row-sale').get_text()),
                            Process.convert_percent(div.select_one('div.new-product-percent').get_text()),
